...
- **Report on truly held-out data.** The LOSO sweep (`scripts/run_loso.sh`, `zte-run --loso-holdout`) evaluates every held-out subject; the evaluation `honesty` block adds a permutation null, a held-out cross-subject decoder, and an anchor-calibration lift for the held-out subject (`zte.evaluation.honesty`, `tests/test_honesty.py`). Portable + auto-GPU + resumable, with `docs/RUNNING.md` and `notebooks/zte_colab.ipynb` (Colab via `uv`).
