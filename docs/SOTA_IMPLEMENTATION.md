...
- `objective.alignment_weight` (`0.1` in both configs). Implemented as `alignment_penalty` in `src/zte/models/objectives/losses.py`, called from `SkipGramObjective.compute` over the final positive-pair mask using the already-L2-normalised center/context embeddings; the scalar `alignment_loss` is logged.
...
- `objective.data2vec_aux_weight` (`0.5` in both configs; requires `model.factored`). The frozen projection and the regression head are built in `_ObjectiveBase.attach_auxiliary` (`data2vec_proj.requires_grad_(False)`) and the cosine regression is applied in `_ObjectiveBase.regularize` over the `_nuisance_slice`, in `src/zte/models/objectives/base.py`; the scalar `data2vec_loss` is logged.
...
- `objective.meaning_contextual` (HF model id, e.g. `bert-base-uncased`; `null` in both flagship configs, which keep the static path) and `objective.meaning_context_layer` (`-1` = last hidden; `~7–9` aligns best with brain data). The target is built by `build_meaning_matrix_hf` in `src/zte/data/targets/meaning.py` — the encoder runs once per unique sentence (`stimulus_key`) and the per-position vectors broadcast to every subject's reading of that word (a ~12× saving and the subject-invariant target LOSO wants). It is plumbed through the `SentenceSample`/collate/pipeline as a per-row `meaning_target`, preferred over the static matrix inside `_ObjectiveBase.regularize` when present.
...
| `objective.subject_adversary_weight`       | `0.1` · `0.1`                               | Gradient-reversal subject adversary (rebalanced) | `models/objectives/` · `_ObjectiveBase.regularize`               | `adv_acc` → chance w/o content drop                              |
| `objective.subject_adversary_warmup_ratio` | `0.3` · `0.3`                               | DANN λ ramp 0→1                                  | `models/objectives/` · `set_progress` / `_adv_lambda`            | `adv_lambda` trace; content lift held                            |
...
| `objective.alignment_weight`               | `0.1` · `0.1`                               | Align+uniformity: alignment half                 | `models/objectives/` · `alignment_penalty`                       | `alignment` health metric ↓                                      |
| `objective.tau_plus`                       | `0.1` · `0.1`                               | Debiased InfoNCE                                 | `models/objectives/` · `debiased_infonce`                        | Same-word retrieval / content lift                               |
| `objective.data2vec_aux_weight`            | `0.5` · `0.5`                               | Frozen-target nuisance regression                | `models/objectives/` · `attach_auxiliary` / `regularize`         | Eff-rank ratio ↑; nuisance occupied                              |
| `objective.meaning_contextual`             | `null` · `null`                             | Per-occurrence contextual target                 | `data/targets/meaning.py` · `build_meaning_matrix_hf`            | Content lift on novel words                                      |
| `objective.meaning_context_layer`          | `-1` · `-1`                                 | Which contextual layer                           | `data/targets/meaning.py` · `build_meaning_matrix_hf`            | (as above)                                                       |
| `objective.meaning_source`                 | `glove.300d.txt`                            | Static fastText/GloVe swap                       | `data/targets/meaning.py` · `build_meaning_matrix`               | Content probe / retrieval lift                                   |
...
| `objective.eval_phase_shuffle`             | `false` · `false`                           | Phase-scrambled-input control                    | `data/features/transforms.py` · `phase_scramble`                 | R² ≈ 0 (band power)                                              |
