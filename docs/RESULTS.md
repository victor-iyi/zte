...
> For methodology see [EVALUATION.md](EVALUATION.md); for the knobs see [TRAINING.md](TRAINING.md) and [DATASET.md](DATASET.md).
...
The raw-Conformer frontend (`--frontend raw_conformer --representation raw`) trains the masked/CPC objectives the same way; see `examples/run_demo.py` and [TRAINING.md](TRAINING.md).
...
See [EVALUATION.md](EVALUATION.md) for what every figure and metric means, plus the per-subject/per-task breakdowns, analogy transfer and scalp-region importance.
...
Point the `--ckpt` / `--root` flags of `zte-evaluate` and `zte-extract` at real artifacts to reproduce these tables on genuine ZuCo recordings.
